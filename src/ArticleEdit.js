...
class ArticleEdit extends Component {
    ...
    async componentDidMount() {
        if (this.props.match.params.id !== 'new') {
            const article = await (await fetch(`http://historie.azurewebsites.net/articles/${this.props.match.params.id}`)).json();
...
async handleSubmit(event) {
    ...
    await fetch('http://historie.azurewebsites.net/articles' + (item.id ? '/' + item.id : ''), {
